...
function init() {
	...
	for ( var i = 0; i < table.length; i += 5 ) {
		...
		$(element).click(function () {
			...
			var startingPoint = getCameraCurrentLookAt();
			
			new TWEEN.Tween( startingPoint )
				.to( targetPosition, 1000 )
				.onUpdate( function() {
					camera.lookAt(startingPoint);
					render(); 
				})
				.start();
		});
	...
	for ( var i = 0, l = objects.length; i < l; i ++ ) {
		...
		object.position.y = ~~(i/36) * 200 - 300;
	...
	// Display of the cards by scaling them on x, then on y. Finally move cards to the helix target
	for ( var i = 0; i < objects.length; i ++ ) {

		var object = objects[ i ];
		var xscaleTween = new TWEEN.Tween( object.scale )
			.to( { x: 1, y: 0.001, z: object.scale.z }, 500)
			.easing( TWEEN.Easing.Exponential.InOut );
		var yscaleTween = new TWEEN.Tween( object.scale )
			.to( { x: 1, y: 1, z: object.scale.z }, 500)
			.easing( TWEEN.Easing.Exponential.InOut );
		xscaleTween.chain(yscaleTween);
		xscaleTween.start();						
	}
	var mainTween = new TWEEN.Tween( this )
		.to( {}, 1000);
	mainTween.onUpdate( render );
	mainTween.start();
	setTimeout(function() {
		transform( helix, 3000 );
	},1000);
